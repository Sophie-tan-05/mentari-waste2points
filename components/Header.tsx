  ...
  return (
    ...
    >
      <div className="max-w-[1160px] mx-auto px-6 py-[14px] flex items-center gap-6">
        {/* Brand */}
        <a href="#top" className="flex items-center gap-[10px] text-white no-underline shrink-0">
        ...
        <nav className="hidden min-[721px]:flex gap-7 ml-[14px]">
          {NAV.map(item => (
            <a
              key={item.id}
              href={`#${item.id}`}
              className="text-[#EAF5EA]/90 hover:text-[#EAF5EA] font-semibold text-[.82rem] tracking-[.1em] uppercase transition-colors no-underline"
            >
        ...
        <div className="ml-auto flex items-center gap-[14px]">
          ...
          <a
            href="#top"
            className="hidden min-[721px]:inline-flex items-center bg-paper text-green font-bold text-[.92rem] px-5 py-[10px] rounded-full hover:bg-white hover:text-green-deep transition-colors whitespace-nowrap no-underline"
          >
        ...
        <div className="min-[721px]:hidden flex flex-col bg-green-deep px-6 pb-[18px] pt-2">
          {NAV.map(item => (
            <a
              ...
              className="text-[#EAF5EA] py-3 font-bold border-b border-white/10 uppercase tracking-[.08em] text-[.82rem] no-underline"
            >
              {lang === 'ms' ? item.ms : item.en}
            </a>
          ))}
